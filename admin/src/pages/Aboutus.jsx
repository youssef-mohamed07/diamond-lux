...
export default function Aboutus() {
  ...
  const validateForm = () => {
    ...
    for (let i = 0; i < formData.whyChoose.length; i++) {
      ...
      if (!item.title.trim()) return `Title is required in Why Choose Us section ${i + 1}`;
      if (!item.text.trim()) return `Text is required in Why Choose Us section ${i + 1}`;
    }
              ...
              {formData.whyChoose.map((item, index) => (
                <div key={index} className="border rounded-md p-2">
                  ...
                  <textarea
                    ...
                    placeholder="Text"
                    value={item.text}
                    onChange={(e) => handleWhyChooseChange(index, 'text', e.target.value)}
                  />
                </div>
              ))}
            </div>
