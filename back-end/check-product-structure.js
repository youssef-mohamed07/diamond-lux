        ...
        while (processedCount < totalCount) {
            ...
            for (const product of products) {
                try {
                    ...
                    Object.entries(requiredStringFields).forEach(([field, defaultValue]) => {
                        if (!product[field]) {
                            needsUpdate = true;
                            if (field === 'title') {
                                updates[field] = `${product.shape || 'Round'} ${product.productType === "lab_diamond" ? "Lab" : "Natural"} Diamond - ${product.carats}ct ${product.col || 'N/A'} ${product.clar || 'N/A'}`;
                            } else if (field === 'description') {
                                updates[field] = `Beautiful ${product.shape || 'Round'} ${product.productType === "lab_diamond" ? "Lab" : "Natural"} Diamond with ${product.carats} carats, ${product.col || 'N/A'} color, and ${product.clar || 'N/A'} clarity.`;
                    ...
                    if (!product.image || !product.imageCover) {
                        needsUpdate = true;
                        const placeholderImage = `https://via.placeholder.com/300x300?text=${encodeURIComponent(product.shape || 'Diamond')}`;
                    ...
                    if (!Array.isArray(product.images) || product.images.length === 0) {
                        needsUpdate = true;
                        updates.images = [updates.image || product.image || `https://via.placeholder.com/300x300?text=${encodeURIComponent(product.shape || 'Diamond')}`];
                    }

                    // Check and fix product type
                    if (!product.productType || !['lab_diamond', 'natural_diamond'].includes(product.productType)) {
                        needsUpdate = true;
                        updates.productType = product.lg === 'natural' ? 'natural_diamond' : 'lab_diamond';
                    }
            ...
            if (bulkOps.length > 0) {
                ...
                console.log(`Processed batch: ${processedCount + 1} to ${processedCount + products.length} (${fixedCount} fixed)`);
            }
